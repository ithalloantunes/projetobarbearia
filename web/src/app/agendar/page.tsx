...
export default function AgendarPage() {
  ...
  const totalSelectedPrice = useMemo(
    () =>
      selectedServices.reduce((sum, service) => {
        const numeric = typeof service.price === "string" ? Number(service.price) : service.price;
  ...
  const compatibleBarbers = useMemo(() => {
    ...
    return barbers.filter((barber) =>
      selectedServiceIds.every((serviceId) => barber.serviceIds.includes(serviceId))
    );
  }, [barbers, selectedServiceIds]);

  const barbersWithAny = useMemo<AppointmentBarber[]>(
    () => (compatibleBarbers.length > 0 ? [...compatibleBarbers, ANY_BARBER_OPTION] : []),
    [compatibleBarbers]
  );
  ...
  const effectiveBarber = useMemo<Barber | null>(() => {
    ...
    if (selectedBarber.id === "any") {
      return compatibleBarbers[0] ?? null;
    }
    return selectedBarber;
  }, [compatibleBarbers, selectedBarber]);

  useEffect(() => {
    if (selectedBarberId === null || selectedBarberId === "any") {
      return;
    }

    const stillCompatible = compatibleBarbers.some((barber) => barber.id === selectedBarberId);
    if (!stillCompatible) {
      setSelectedBarberId(null);
    }
  }, [compatibleBarbers, selectedBarberId]);
  ...
  useEffect(() => {
    ...
    async function loadBase() {
      ...
      try {
        ...
        const serviceNameFromQuery = query.get("serviceName") || "";
        const servicesFromQuery = query.get("services") || "";
        const serviceNamesFromQuery = query.get("serviceNames") || "";
        const barberFromQuery = query.get("barber") || "";
        const barberNameFromQuery = query.get("barberName") || "";

        const serviceCandidates = [
          serviceFromQuery,
          ...servicesFromQuery.split(","),
          serviceNameFromQuery,
          ...serviceNamesFromQuery.split("|")
        ]
        ...
        for (const candidate of serviceCandidates) {
          ...
          const matchedService = nextServices.find((service) => {
            ...
            return slug === normalizedCandidate || String(service.id) === candidate;
          });

          if (matchedService && !matchedServices.some((service) => service.id === matchedService.id)) {
            matchedServices.push(matchedService);
          }
        }

        const barberCandidates = [barberFromQuery, barberNameFromQuery]
          .map((value) => value.trim())
          .filter(Boolean);
        ...
        for (const candidate of barberCandidates) {
          ...
          const possibleMatch =
            nextBarbers.find((barber) => {
              ...
              return slug === normalizedCandidate || String(barber.id) === candidate;
            }) || null;

          if (possibleMatch) {
            matchedBarber = possibleMatch;
            break;
          }
        }

        const primaryMatchedService = matchedServices[0] ?? null;
        const nextSelectedServiceIds =
          matchedServices.length > 0
            ? matchedServices.map((service) => service.id)
            : nextServices[0]?.id
              ? [nextServices[0].id]
              : [];

        const compatibleBarbersForPrefill =
          nextSelectedServiceIds.length === 0
            ? nextBarbers
            : nextBarbers.filter((barber) =>
                nextSelectedServiceIds.every((serviceId) => barber.serviceIds.includes(serviceId))
              );

        const matchedBarberCompatible =
          matchedBarber && compatibleBarbersForPrefill.some((barber) => barber.id === matchedBarber.id)
            ? matchedBarber
            : null;

        const nextSelectedBarberId =
          matchedBarberCompatible?.id ?? compatibleBarbersForPrefill[0]?.id ?? nextBarbers[0]?.id ?? null;
        ...
        if (primaryMatchedService && matchedBarberCompatible) {
          setCurrentStep(3);
          setPrefillNotice(
            matchedServices.length > 1
              ? `${matchedServices.length} servicos foram pre-selecionados na home.`
              : "Selecao inicial aplicada com base na sua escolha da pagina inicial."
          );
        } else if (primaryMatchedService && matchedBarber && !matchedBarberCompatible) {
          setCurrentStep(2);
          setPrefillNotice("O barbeiro pre-selecionado nao atende todos os servicos escolhidos. Escolha outro para continuar.");
        } else if (primaryMatchedService) {
          setCurrentStep(2);
  ...
  useEffect(() => {
    async function loadAvailability() {
      if (selectedServices.length === 0 || !effectiveBarber || !selectedDate) {
        setAvailableTimes([]);
        setSelectedTime("");
        return;
      }
      ...
      try {
        ...
        const primaryServiceId = primarySelectedService?.id;
        const serviceIdsQuery = selectedServices.map((service) => String(service.id)).join(",");
        const response = await fetch(
          `/api/availability?barberId=${effectiveBarber.id}&serviceId=${primaryServiceId}&serviceIds=${serviceIdsQuery}&date=${date}`
        );
  ...
  }, [selectedServices, primarySelectedService, effectiveBarber, selectedDate]);

  async function handleSubmit() {
    setError(null);
    setMessage(null);

    if (selectedServices.length === 0 || !effectiveBarber || !selectedTime) {
    ...
    const primaryServiceId = primarySelectedService?.id;
    if (!primaryServiceId) {
      setError("Selecione ao menos um servico.");
      return;
    }
        ...
        {isSubmitted ? (
          <Panel className="text-center">
            ...
            <p className="mt-2 text-sm text-foreground-muted">
              {selectedServices.map((service) => service.name).join(" + ") || "-"} com {effectiveBarber?.name ?? "-"} em{" "}
                  ...
                  {loadingBase ? (
                    ...
                    <div className="space-y-3">
                      <p className="text-sm text-foreground-muted">
                        Voce pode selecionar mais de um servico nesta etapa.
                      </p>
                ...
                <Panel title="Confirme sua reserva">
                  <div className="space-y-3 text-sm">
                    <div className="atelier-surface p-3">
                      <p className="atelier-label">Servicos</p>
                    ...
                    <div className="atelier-surface p-3">
                      ...
                      <p className="font-semibold text-primary">
                        R$ {selectedServices.length > 0 ? formatPrice(totalSelectedPrice) : "-"} (
                        {selectedServices.length > 0 ? totalSelectedDuration : "-"} min)
                      </p>
                    </div>
                    {userRole === "ADMIN" ? (
              ...
              <div className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground-muted">
                    Duracao total: {selectedServices.length > 0 ? totalSelectedDuration : "-"} min
                  </p>
                </div>
              </div>
            </Panel>
