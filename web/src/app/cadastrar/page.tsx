...
export default function CadastrarPage() {
  ...
  useEffect(() => {
    ...
    const primaryServiceName = params.get("serviceName") || "";
    const serviceNamesFromQuery = (params.get("serviceNames") || "")
      .split("|")
      .map((value) => value.trim())
      .filter(Boolean);

    const nextServiceNames = primaryServiceName
      ? [primaryServiceName, ...serviceNamesFromQuery.filter((value) => value !== primaryServiceName)]
  ...
  return (
    ...
    >
      <form className="space-y-4" onSubmit={handleSubmit}>
        {serviceNames.length > 0 || barberName ? (
