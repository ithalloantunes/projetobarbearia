...
## Status de execucao atualizado (2026-03-25)
...
1. P2 - observabilidade avancada externa (Sentry/APM + alertas com on-call) ainda pendente.
2. P2 - restore testado em homologacao e validacao formal de continuidade ainda pendentes.
3. P2 - revisao de performance em paginas chave ainda pendente.
4. Legal/compliance - mapear processo formal de solicitacao/exclusao de dados LGPD.

## Proxima frente sugerida (ordem)

1. Executar teste de restore em homologacao e registrar evidencias de RTO.
2. Iniciar piloto controlado do upgrade Prisma 7 usando o runbook de rollback.
3. Evoluir observabilidade para stack externa (Sentry/APM + alertas de on-call).

## Nota tecnica sobre dependencias (2026-03-25)

Para eliminar vulnerabilidades altas reportadas no `npm audit`, Prisma foi fixado em `6.12.0` (faixa sem advisory ativo no lock atual).
O upgrade para Prisma 7 continua planejado como evolucao estruturante, com migracao dedicada e janela de rollback.
