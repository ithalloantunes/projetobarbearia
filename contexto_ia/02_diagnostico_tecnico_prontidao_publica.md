...
1. rollout canario de Prisma 7 e aprovacao formal de rollback ainda pendentes.
2. observabilidade avancada externa (Sentry/APM + alertas on-call) pendente de configuracao de DSN/alertas.
3. performance de paginas chave e fechamento operacional de LGPD/go-live ainda pendentes.

## Adendo de progresso (2026-03-31)

1. piloto controlado de Prisma 7 foi executado em homologacao local com sucesso.
2. stack Prisma foi adaptada para o novo modelo:
   - `datasource.url` movido para `prisma.config.ts`;
   - `PrismaClient` atualizado para uso de driver adapter (`@prisma/adapter-mariadb`).
3. validacoes executadas no piloto:
   - `verify` em verde;
